...
func (c *Controller) cardVisitorActivity(data ControllerData) hb.TagInterface {
	return hb.Div().
		...
		Child(hb.Div().
			Class("card-body").
			Child(c.tableVisitorActivity(data, data.visitors)).
			Child(c.pagination(data, data.page, data.totalPages)))
}

// tableVisitorActivity creates the visitor activity table
func (c *Controller) tableVisitorActivity(data ControllerData, visitors []statsstore.VisitorInterface) hb.TagInterface {
	table := hb.Table().
...
func (c *Controller) pagination(data ControllerData, page int, totalPages int) hb.TagInterface {
	if totalPages <= 1 {
		return hb.Div()
	}
