...
func TestHandleDailyAjaxSuccess(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/admin/home?action=daily-ajax&period=this-week", nil)
	...
	if !strings.Contains(body, "dailyStats") {
		t.Fatalf("expected dailyStats in response, got: %s", body)
	}
	if !strings.Contains(body, "chartLabels") {
		t.Fatalf("expected chartLabels in response, got: %s", body)
	}
}

func TestHandleDailyAjaxError(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/admin/home?action=daily-ajax", nil)
	rr := httptest.NewRecorder()
	controller.ServeHTTP(rr, req)
