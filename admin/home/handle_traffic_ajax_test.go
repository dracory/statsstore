...
func TestHandleTrafficAjaxSuccess(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/admin/home?action=traffic-ajax&period=this-week", nil)
	...
	body := rr.Body.String()
	if !strings.Contains(body, "trafficCards") {
		t.Fatalf("expected trafficCards in response, got: %s", body)
	}
}

func TestHandleTrafficAjaxError(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/admin/home?action=traffic-ajax", nil)
	rr := httptest.NewRecorder()
	controller.ServeHTTP(rr, req)
